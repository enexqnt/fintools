...
def main():
    ...
    st.title('Relaxed Risk Parity Optimization')
    ...
    st.write('Based on the paper by Gambeta and Kwon: Risk return trade-off in relaxed risk parity portfolio optimization (2020) https://doi.org/10.3390/jrfm13100237')
    ...
    md=st.selectbox('Select the model:',['A','B','C'])
    lower_return=st.slider('Define the minimum required return (in percentage)',min_value=1,max_value=8)
    ##############################
    ...
    if st.button('Run optimization'):
        try:
            ...
            for i in index_:
                    ...
                    port.lowerret = (lower_return/100+1)**(1/252)-1
                    w = port.rrp_optimization(model='Classic',version=md)
            ...
            st.write('You must define the inputs')
